RSpec.describe "bulk_email" do
  ...
  describe "temp_incident_new_subscribers" do
    ...
    it "sends a single apology email to affected subscribers" do
      # affected emails
      create(:email, subject: affected_subject, created_at: Time.new(2021, 1, 7, 14, 35))
      create(:email, subject: affected_subject, created_at: Time.new(2021, 1, 8, 13, 40))

      # unaffected emails
      create(:email, subject: affected_subject, created_at: Time.new(2021, 1, 7, 14, 30))
      create(:email, subject: affected_subject, created_at: Time.new(2021, 1, 8, 13, 50))
      create(:email, subject: "something else", created_at: Time.new(2021, 1, 8, 13, 40))
      ...
      expect(apology_emails.first.address).to eq(Email.distinct.pluck(:address).first)
    end
  end
end
